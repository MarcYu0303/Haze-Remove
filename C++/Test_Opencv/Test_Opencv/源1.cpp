...
void HazeRemoval(Mat& image, Mat& imageRGB)
{
	...
	std::vector<double> A(3);                //分别存取B,G,R通道的最大A值
	std::vector<Mat> fImageBorderVectorA(3);//在求第三步的t(x)时，会用到以下的矩阵，这里可以提前求出
...
int main()
{
	Mat image = imread("D://360MoveData//Users//MarcYu//Desktop//DIP project//db//IEI2019//H26.jpg");//设置0写入灰度图像
